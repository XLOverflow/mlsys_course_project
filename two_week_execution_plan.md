...
**Day 1 硬阻塞项（不解决就不要调度 GPU）**：

- `torch.fx.symbolic_trace` 在 HF 的 GPT-2/BERT/T5 上大概率挂（`if input_ids is not None`、causal mask、tuple 返回会炸）。改用 `transformers.utils.fx.symbolic_trace(model, input_names=["input_ids"])` 替换 [extractor.py:39](src/hetero_cost_model/graph/extractor.py#L39)
- Day 1 下班前必须跑通 6 个模型的 CPU fx smoke test（gpt2-small/medium/large、bert-base/large、t5-small）
- T5 带 past_key_values 时即使 HF fx 也可能失败。若 smoke test 挂，**当场决定**：丢掉 T5 或改 ONNX 导出路径。不要带着裂的 trace 进入 Day 3 profiling

| 负责人 | 任务 | 交付物 | 需要GPU |
|--------|------|--------|---------|
| **Xiang Li** | (a) 完成 fx smoke test（Day 1 必交）；(b) 用 HF fx 修复 [extractor.py](src/hetero_cost_model/graph/extractor.py) 节点特征提取；(c) 增加 FP16 精度下 `memory_bytes` 的正确估算（当前 [extractor.py:74](src/hetero_cost_model/graph/extractor.py#L74) 硬编码 fp32） | `graph_extractor.py` + smoke test | 否 |
| **Xinhao Tan** | 构建 profiling 框架：(a) `torch.cuda.Event(enable_timing=True)` 替换 [profiling.py:51](src/hetero_cost_model/profiling.py#L51) 的 `time.perf_counter`；(b) warmup=50、runs=100；(c) 训练 target 改用 **p50** 而非 mean；(d) 全部模型 `attn_implementation="eager"`；(e) 增加 `actual_gpu_name` / `actual_mem_gb` / `actual_sm_count` 写入 CSV | `profiler.py` + `config_grid.py` | 否（本地 CPU 调试） |
| **Zhikai Hu** | 用 PyG 实现 GNN 代价模型（GAT 主架构 + Graph Transformer 变体）；定义硬件特征向量 schema；搭建训练循环（MSE + ranking loss）；**同时实现 XGBoost baseline**（全局手工特征：total_flops、total_bytes、batch、seq_len、5 维 h） | `cost_model.py` + `train.py` + `xgb_baseline.py` | 否 |

**Day 2 检查点**：三人在本地完成各自模块的单元测试，确认接口可以对接。**关键红绿灯**：fx smoke test 6/6 通过才算绿灯进入 Day 3。
...
**"H200 改成 few-shot"的原因**：评审发现 H200 vs H100 在 h 向量中只有 `memory_gb` 和 `bandwidth_gbs` 不同（`fp16_tflops / pcie_gbs / sm_count` 完全相等），compute-bound 区 latency 本就 ≈ H100，zero-shot 在只有 3 个训练 anchor 时几乎是白送的。few-shot 更诚实。详见 [research_review.md §2.1](research_review.md)。
...
| 风险 | 影响 | 应对策略 |
|------|------|----------|
| **`torch.fx.symbolic_trace` 挂在 HF 模型上** | **Day 1 单点故障**，会让整条数据管线废掉 | Day 1 CPU smoke test 必过，改用 `transformers.utils.fx.symbolic_trace(model, input_names=["input_ids"])`。T5 挂就当场砍掉或走 ONNX |
