...
> 基于 [plan.md](plan.md)、[methods.md](methods.md)、`src/hetero_cost_model/` 代码的综合评审
...
详细 Phase 6 结果：[archive/phase6-7gpu-sweep.md](archive/phase6-7gpu-sweep.md) §6.1–§6.10（**已 superseded by 5-GPU 终版**，见 [results.md](results.md)）。
...
3. ~~**代码里有一个能直接报废两周的单点故障**：`torch.fx.symbolic_trace` 在 HF 的 GPT-2/BERT/T5 上会炸。~~ **✅ 2026-04-17 已解决**：改走 `transformers.utils.fx.symbolic_trace` + pin `transformers>=4.35,<4.52` + smoke test（`scripts/smoke_test_graphs.py`，6/6 PASS）。守护措施见 §3.1。
4. **剩余关键动作**：(A) ~~6 个模型 fx tracing smoke test~~ **已完成**；(B) 重新设计评估协议（leave-one-GPU-out CV + 换强基线 XGBoost）仍待做。
...
6 模型 × 5 GPU × 9 配置 = 270 条（详见 [methods.md §1.1](methods.md#11-sample-definition)）。
...
原 scope 决策（[plan.md §1.1](plan.md#11-scope-decisions-vs-proposal-must-declare-in-intro)）明确 prefill 是主 protocol，decoder 只做单次 forward。Decode-mode prediction 后来在 `decode-exploration` 分支补做（见 [results.md §1.2](results.md#12-decode-leave-model-out-3-splits)）。
...
### 3.1 `torch.fx.symbolic_trace` 在 HF 模型上 —— ✅ 已解决（2026-04-17）

**原隐患**：[extractor.py:39](src/hetero_cost_model/graph/extractor.py#L39) 初版用裸 `fx.symbolic_trace(model)`，会在 GPT-2/BERT/T5 上因 `if input_ids is not None`、causal mask 构造、tuple 返回挂掉。曾被列为**能直接报废两周的单点故障**。

**解决方案**（三重护栏）：

1. **代码**：[extractor.py](src/hetero_cost_model/graph/extractor.py) 新增 `hf_input_names` 参数，HF 模型走 `transformers.utils.fx.symbolic_trace(model, input_names=[...])`
2. **版本 pin**：`transformers>=4.35,<4.52`（写入 [requirements.txt](requirements.txt) + [environment.yml](environment.yml)）。5.x 删了 `transformers.utils.fx` 模块；4.52+ 的 `masking_utils.py` 用 `torch.vmap`，与 HF fx 的 `HFProxy` 不兼容。4.46.3 已验证
3. **Smoke test**：[scripts/smoke_test_graphs.py](scripts/smoke_test_graphs.py) 对 6 个模型全量 CPU 调用 `extract_graph`，当前 **6/6 PASS**

**守护措施（必须保留，不要删）**：

- 不要升级 transformers 到 ≥ 4.52
- 每次环境重装 / 新成员入组 / transformers 版本变动后，**必须先跑 smoke test**：`python scripts/smoke_test_graphs.py`。不通过不得进入 profiling
- 加新模型到训练集时，先在 smoke test 的 `MODELS` 列表里加一条验证
...
| `std/mean < 0.05` 质检标准在 shared host 上常失败 | profiling 实践 ([methods.md §1.4](methods.md#14-profiling-protocol)) | 保留噪声行但打 `noisy=True` 标签，训练时过滤；用 **p50** 作为训练 target，长尾异常自然被中位数滤掉 |
...
- **SXM vs PCIe 形态**：Modal H100 是 SXM 版（700W、NVLink），PSC H100 大概率也是 SXM。早期版本的 `HARDWARE_REGISTRY` 曾含 `pcie_gbs` 字段，因 NVLink 在单 GPU forward-pass 里不参与计算、signal 为零，**2026-04-17 改为 `l2_cache_mb`**（跟 NeuSight ASPLOS'25 对齐，只用片上特征）。SXM vs PCIe 的区分现已和硬件向量解耦，仅在 profiling session 记录到 CSV 的 `actual_gpu_name` 列用于事后分析
...
### (A) Day 1：fx tracing 验证（不上 GPU 之前）—— ✅ 已完成 2026-04-17

1. ✅ `transformers.utils.fx.symbolic_trace` 已接入 [extractor.py:56-69](src/hetero_cost_model/graph/extractor.py#L56-L69)（通过新增 `hf_input_names` 参数）
2. ✅ [scripts/smoke_test_graphs.py](scripts/smoke_test_graphs.py) 跑 gpt2-small/medium/large、bert-base/large、t5-small，**6/6 PASS**
3. ✅ T5 通过（用 `["input_ids", "decoder_input_ids"]`，`T5ForConditionalGeneration` + `attn_implementation="eager"`），不需要 ONNX 回退
